...
class DynamicJsonToTemplateConverter extends RepresentationConverter_1.RepresentationConverter {
    ...
    async handle(input) {
        ...
        if (type === ContentTypes_1.APPLICATION_JSON && typeMap[ContentTypes_1.APPLICATION_JSON].length === 0) {
            return representation;
        }
        const contents = JSON.parse(await (0, StreamUtil_1.readableToString)(representation.data));
    ...
    constructTypeMap(identifier, representation) {
        ...
        if (templates.length === 0) {
            throw new NotImplementedHttpError_1.NotImplementedHttpError('No templates found.');
        }
        ...
        if (!typeMap[ContentTypes_1.APPLICATION_JSON]) {
            typeMap[ContentTypes_1.APPLICATION_JSON] = '';
        }
        return typeMap;
    }
    /**
    ...
    findType(typeMap, typePreferences = {}) {
        ...
        const type = (0, ConversionUtil_1.getConversionTarget)(typeWeights, typePreferences);
        if (!type) {
            throw new NotImplementedHttpError_1.NotImplementedHttpError(`No templates found matching ${Object.keys(typePreferences).join(',')}, only ${Object.keys(typeMap).join(',')}`);
        }
        return type;
    }
}
