...
class ParameterResolver {
    constructor(args) {
        ...
        this.cacheInterfaceRange = new LRUCache(2048);
    }
    /**
     ...
     */
    async resolveAllConstructorParameters(unresolvedParametersIndex) {
        const resolvedParametersIndex = {};
     ...
     */
    async resolveConstructorParameters(unresolvedConstructorData) {
        return {
            parameters: (await this.resolveParameterData(unresolvedConstructorData.parameters, unresolvedConstructorData.classLoaded, {}, new Set())).filter(parameter => parameter.type === 'field'),
     ...
     * @param unresolvedParametersIndex An index of unresolved constructor data.
     */
    async resolveAllGenericTypeParameterData(unresolvedParametersIndex) {
        const resolvedGenericsIndex = {};
        // Resolve parameters for the different constructors in parallel
        await Promise.all(Object.entries(unresolvedParametersIndex)
            .map(async ([className, unresolvedGenericsData]) => {
            resolvedGenericsIndex[className] = {
                genericTypeParameters: await this.resolveGenericTypeParameterData(unresolvedGenericsData.genericTypeParameters, unresolvedGenericsData.classLoaded, {}),
     ...
     * @param unresolvedParametersIndex An index of unresolved constructor data.
     */
    async resolveAllMemberParameterData(unresolvedParametersIndex) {
        const resolvedIndex = {};
        // Resolve parameters for the different constructors in parallel
        await Promise.all(Object.entries(unresolvedParametersIndex)
     ...
     * @param genericTypeRemappings A remapping of generic type names.
     */
    async resolveMemberParameterData(members, owningClass, genericTypeRemappings) {
     ...
     */
    async resolveParameterData(parameters, owningClass, genericTypeRemappings, handlingInterfaces) {
     ...
     */
    async resolveAllExtensionData(unresolvedExtensionData, classIndex) {
        const resolvedIndex = {};
        // Resolve parameters for the different constructors in parallel
     ...
     * @param genericTypeRemappings A remapping of generic type names.
     */
    async resolveExtensionData(extensionDatas, owningClass, genericTypeRemappings) {
        return await Promise.all(extensionDatas.map(async (extensionData) => ({
            ...
            genericTypeInstantiations: await Promise.all(extensionData.genericTypeInstantiations
                .map(async (genericTypeInstantiation) => await this.resolveRange(genericTypeInstantiation, owningClass, genericTypeRemappings, false, new Set()))),
        })));
    }
    isIgnored(qualifiedPath, className) {
     ...
     * @param handlingInterfaces The names of interfaces that are being handled, and this interface is a part of.
     */
    async resolveRange(range, owningClass, genericTypeRemappings, getNestedFields, handlingInterfaces) {
        switch (range.type) {
            case 'raw':
            case 'literal':
            case 'override':
            ...
            case 'interface':
                ...
                return await this.resolveRangeInterface(range.value, range.qualifiedPath, range.genericTypeParameterInstantiations, range.origin, owningClass, genericTypeRemappings, getNestedFields, handlingInterfaces);
            case 'hash':
                return {
                    type: 'nested',
                    value: await this
     ...
     */
    hashParameterRangeUnresolved(range) {
        switch (range.type) {
            case 'undefined':
                return 'undefined';
            case 'wildcard':
     ...
     * @param handlingInterfaces The names of interfaces that are being handled, and this interface is a part of.
     */
    resolveRangeInterface(interfaceName, qualifiedPath, genericTypeParameterInstances, owningClass, rootOwningClass, genericTypeRemappings, getNestedFields, handlingInterfaces) {
        ...
        if (!promise) {
            promise = this.resolveRangeInterfaceInner(interfaceName, qualifiedPath, genericTypeParameterInstances, owningClass, rootOwningClass, genericTypeRemappings, getNestedFields, handlingInterfaces);
            this.cacheInterfaceRange.set(cacheKey, promise);
        }
        return promise;
    }
    async resolveRangeInterfaceInner(interfaceName, qualifiedPath, genericTypeParameterInstances, owningClass, rootOwningClass, genericTypeRemappings, getNestedFields, handlingInterfaces) {
        const classOrInterface = await this.loadClassOrInterfacesChain({
            packageName: owningClass.packageName,
            ...
            return {
                ...
                genericTypeParameterInstances: genericTypeParameterInstances ?
                    await Promise.all(genericTypeParameterInstances
                        .map(genericTypeParameter => this.resolveRange(genericTypeParameter, rootOwningClass, genericTypeRemappings, getNestedFields, handlingInterfaces))) :
                    undefined,
            };
        }
        ...
        if (classOrInterface.type === 'type') {
            ...
            const unresolvedFields = this.parameterLoader.getRangeFromTypeNode(classOrInterface, classOrInterface.declaration.typeAnnotation, `type alias ${classOrInterface.localName} in ${classOrInterface.fileName}`);
            return this
        ...
        if (classOrInterface.type === 'enum') {
            const enumRangeTypes = await Promise.all(classOrInterface.declaration.members
                .map((enumMember, i) => {
                if (enumMember.initializer && enumMember.initializer.type === typescript_estree_1.AST_NODE_TYPES.Literal) {
                    return this.resolveRange(this.parameterLoader.getRangeFromTypeNode(classOrInterface, {
                        type: typescript_estree_1.AST_NODE_TYPES.TSLiteralType,
                        literal: enumMember.initializer,
                        loc: undefined,
                        range: undefined,
                    }, `enum ${classOrInterface.localName} in ${classOrInterface.fileName}`), owningClass, genericTypeRemappings, getNestedFields, handlingInterfaces);
                }
     ...
     */
    isInterfaceImplicitClass(iface) {
        for (const field of iface.declaration.body.body) {
            if (field.type === typescript_estree_1.AST_NODE_TYPES.TSMethodSignature ||
                field.type === typescript_estree_1.AST_NODE_TYPES.TSConstructSignatureDeclaration ||
                (field.type === typescript_estree_1.AST_NODE_TYPES.TSPropertySignature && field.typeAnnotation &&
                    field.typeAnnotation.typeAnnotation.type === typescript_estree_1.AST_NODE_TYPES.TSFunctionType)) {
                return true;
            }
        }
        return false;
     ...
     */
    async loadClassOrInterfacesChain(classReference) {
        ...
        if (classOrInterface.type === 'type' &&
            classOrInterface.declaration.typeAnnotation.type === typescript_estree_1.AST_NODE_TYPES.TSTypeReference &&
            classOrInterface.declaration.typeAnnotation.typeName.type === typescript_estree_1.AST_NODE_TYPES.Identifier) {
     ...
     * @param handlingInterfaces The names of interfaces that are being handled, and this interface is a part of.
     */
    async getNestedFieldsFromInterface(iface, owningClass, genericTypeRemappings, handlingInterfaces) {
     ...
     * @param handlingInterfaces The names of interfaces that are being handled, and this interface is a part of.
     */
    async getNestedFieldsFromHash(hash, owningClass, genericTypeRemappings, handlingInterfaces) {
