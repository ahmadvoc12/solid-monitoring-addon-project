...
exports.ComponentConstructor = void 0;
const semverMajor = require("semver/functions/major");
const ContextConstructor_1 = require("./ContextConstructor");
/**
...
class ComponentConstructor {
    ...
    async constructComponents() {
        ...
        const context = await this.contextParser.parse(this.contextConstructor.constructContext());
        for (const [className, classReference] of Object.entries(this.classAndInterfaceIndex)) {
            ...
            components.push(await this.constructComponent(context, externalContextUrl => {
                // Append external contexts URLs to @context array
                if (!contexts.includes(externalContextUrl)) {
                    contexts.push(externalContextUrl);
                }
            }, classReference, this.classConstructors[className], this.classGenerics[className], this.classExtensions[className], (_a = this.classMembers[className]) === null || _a === void 0 ? void 0 : _a.members));
        }
        return definitions;
    }
    /**
     ...
     */
    async constructComponentsIndex(definitions) {
        // Construct a minimal context
        const context = await this.contextParser.parse(this.contextConstructor.constructContext());
        return {
            '@context': Object.keys(this.packageMetadata.contexts),
     ...
     */
    static getPathRelative(pathDestination, sourcePath) {
     ...
     * @param sourcePath The absolute path to a class file.
     */
    static getPathDestination(pathDestination, sourcePath) {
        if (!sourcePath.startsWith(pathDestination.packageRootDirectory)) {
            throw new Error(`Tried to reference a file outside the current package: ${sourcePath}`);
        }
     ...
     */
    async constructComponent(context, externalContextsCallback, classReference, constructorData, genericsData, classExtensions, classMembers) {
        // Determine generic type parameters
        const genericTypeParameters = genericsData ?
            await this.constructGenericTypeParameters(context, externalContextsCallback, classReference, genericsData.genericTypeParameters) :
            undefined;
        ...
        const ext = classExtensions && classExtensions.length > 0 ?
        ...
        const memberFields = classMembers ?
            await this.constructMembers(context, externalContextsCallback, classReference, classMembers) :
            undefined;
        // Fill in fields
        return {
            '@id': scopedId,
     ...
     */
    async constructExtensionDefinition(context, externalContextsCallback, extensionData) {
        ...
        if (extensionData.genericTypeInstantiations.length > 0) {
            return {
                ...
                genericTypeInstances: await Promise.all(extensionData.genericTypeInstantiations
                    .map(async (genericTypeInstantiation) => this.constructParameterRange(genericTypeInstantiation, context, externalContextsCallback, ''))),
            };
        }
        return id;
    }
     ...
     */
    async classNameToId(context, externalContextsCallback, classReference) {
        // Mint a new IRI if class is in the current package
        if (classReference.packageName === this.packageMetadata.name) {
            return ComponentConstructor.classNameToIdForPackage(context, this.packageMetadata, this.pathDestination, classReference, this.fileExtension);
        }
        ...
        if (otherPackageMetadata) {
            ...
            return await ComponentConstructor.classNameToIdForPackage(otherPackageMetadata.minimalContext, otherPackageMetadata.packageMetadata, otherPackageMetadata.pathDestination, classReference, this.fileExtension);
        }
    ...
    static async classNameToIdForPackage(context, packageMetadata, pathDestination, classReference, fileExtension) {
    ...
    static classNameToIriForPackage(packageMetadata, pathDestination, classReference, fileExtension) {
        const filePath = ComponentConstructor.getPathRelative(pathDestination, ComponentConstructor.getPathDestination(pathDestination, classReference.fileName));
     ...
     */
    fieldNameToId(context, classReference, fieldName, scope) {
        ...
        let iri;
        if (classReference.packageName === this.packageMetadata.name) {
            iri = ComponentConstructor.classNameToIriForPackage(this.packageMetadata, this.pathDestination, classReference, this.fileExtension);
        }
        // Use existing IRI if class is in another package (that is also being built currently)
        const otherPackageMetadata = this.externalComponents.packagesBeingGenerated[classReference.packageName];
        if (otherPackageMetadata) {
            iri = ComponentConstructor.classNameToIriForPackage(otherPackageMetadata.packageMetadata, otherPackageMetadata.pathDestination, classReference, this.fileExtension);
        }
        if (!iri) {
     ...
     */
    memberToId(context, classReference, memberName) {
     ...
     */
    async constructMembers(context, externalContextsCallback, classReference, members) {
        const definitions = [];
     ...
     */
    async constructParameters(context, externalContextsCallback, classReference, constructorData, parameters) {
        const scope = {
            parentFieldNames: [],
            fieldIdsHash: {},
            defaultNested: [],
        };
        return await Promise.all(constructorData.parameters.map(parameter => this.parameterDataToConstructorArgument(context, externalContextsCallback, classReference, parameter, parameters, this.fieldNameToId(context, classReference, parameter.name, scope), scope)));
    }
    /**
     ...
     * @param scope The current field scope.
     */
    async parameterDataToConstructorArgument(context, externalContextsCallback, classReference, parameterData, parameters, fieldId, scope) {
        ...
        if (parameterData.range.type === 'nested' ||
            ...
            const fields = [];
            const subParams = parameterData.range.type === 'nested' ?
                parameterData.range.value :
                parameterData.range.elements.find(element => element.type === 'nested').value;
            for (const subParamData of subParams) {
                fields.push(await this.constructFieldDefinitionNested(context, externalContextsCallback, classReference, parameterData, parameters, subParamData, fieldId, scope));
            }
            return { '@id': `${fieldId}__constructorArgument`, fields };
        }
        // Check if we have a defaultNested value that applies on this field
        const defaultValues = parameterData.defaults || [];
        ...
        const defaultValuesConstructed = await Promise.all(defaultValues
            .map(defaultValue => this.constructDefaultValueDefinition(fieldId, context, externalContextsCallback, defaultValue, parameterData.range)));
        ...
        const param = {
            ...
            ...defaultValuesConstructed.length > 0 ?
                {
                    default: defaultValuesConstructed.length > 1 || ContextConstructor_1.ContextConstructor.isParameterRangeList(range) ?
    ...
    async constructDefaultValueDefinition(fieldId, context, externalContextsCallback, defaultValue, range) {
        if (defaultValue.type === 'raw') {
            if ((range.type === 'override' && range.value === 'json') ||
                ...
                try {
                    ...
                    throw new Error(`JSON parsing error in default value of ${fieldId}: ${error.message}`);
                }
            }
     ...
     * @param scope The current field scope.
     */
    async constructFieldDefinitionNested(context, externalContextsCallback, classReference, parameterData, parameters, subParamData, fieldId, scope) {
        if (subParamData.type === 'field') {
            return {
                keyRaw: subParamData.name,
                value: await this.parameterDataToConstructorArgument(context, externalContextsCallback, classReference, subParamData, parameters, this.fieldNameToId(context, classReference, subParamData.name, scope), scope),
            };
        }
        // Handle index type
        // Indexed elements can only occur in a field
        if (parameterData.type === 'index') {
            throw new Error(`Detected illegal indexed element inside a non-field in ${classReference.localName} at ${classReference.fileName}`);
        }
        ...
        const value = await this.parameterDataToConstructorArgument(context, externalContextsCallback, classReference, subParamData, subParameters, idValue, scope);
     ...
     * @param fieldId The @id of the field.
     */
    async constructParameterRange(range, context, externalContextsCallback, fieldId) {
        let type;
        switch (range.type) {
            case 'raw':
            case 'override':
     ...
     */
    populateOptionalParameterFields(parameterDefinition, parameterData) {
...
//# sourceMappingURL=ComponentConstructor.js.map