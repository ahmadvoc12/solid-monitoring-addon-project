 ...
 */
export declare const INDEX_ID_KEY = "id";
/**
 ...
 */
export type ValueTypeDescription<TType = string> = `${('string' | 'boolean' | 'number' | (TType extends string ? `${typeof INDEX_ID_KEY}:${TType}` : never))}${'[]' | ''}${'?' | ''}`;
/**
 * Converts a {@link ValueTypeDescription} to the type it should be interpreted as.
 */
export type ValueType<T extends ValueTypeDescription> = (T extends `${infer E extends ValueTypeDescription}[]${string}` ? ValueType<E>[] : T extends `boolean${string}` ? boolean : T extends `number${string}` ? number : string) | (T extends `${string}?` ? undefined : never);
/**
 ...
 */
export type TypeObject<TDesc extends Record<string, ValueTypeDescription>> = IndexObject<TDesc> & {
    [INDEX_ID_KEY]: string;
};
/**
...
export type IndexedQuery<T extends IndexTypeCollection<T>, TType extends keyof T, TDepth extends number = 10> = [
    ...
    [K in keyof T[TType] | typeof INDEX_ID_KEY]?: ValueType<T[TType][K]> | (T[TType][K] extends `${typeof INDEX_ID_KEY}:${infer U}` ? IndexedQuery<T, U, Prev[TDepth]> : never);
};
/**
