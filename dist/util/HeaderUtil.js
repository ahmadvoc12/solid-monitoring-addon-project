...
exports.hasScheme = exports.matchesAuthorizationScheme = exports.parseLinkHeader = exports.parseForwarded = exports.parseContentType = exports.addHeader = exports.parseAcceptDateTime = exports.parseAcceptLanguage = exports.parseAcceptEncoding = exports.parseAcceptCharset = exports.parseAccept = exports.parseParameters = exports.splitAndClean = exports.transformQuotedStrings = void 0;
const escape_string_regexp_1 = __importDefault(require("escape-string-regexp"));
const LogUtil_1 = require("../logging/LogUtil");
const BadRequestHttpError_1 = require("./errors/BadRequestHttpError");
const Header_1 = require("./Header");
const logger = (0, LogUtil_1.getLoggerFor)('HeaderUtil');
...
function transformQuotedStrings(input) {
    let idx = 0;
    const replacements = {};
    const result = input.replaceAll(/"(?:[^"\\]|\\.)*"/gu, (match) => {
        // Not all characters allowed in quoted strings, see BNF above
        if (!Header_1.QUOTED_STRING.test(match)) {
 ...
 */
function handleInvalidValue(message, strict) {
    logger.warn(message);
    if (strict) {
        throw new BadRequestHttpError_1.BadRequestHttpError(message);
    }
}
/**
 ...
 */
function parseParameters(parameters, replacements, strict = false) {
    const parsed = [];
    for (const param of parameters) {
        ...
        if (!(Header_1.TOKEN.test(name) && (!rawValue || /^"\d+"$/u.test(rawValue) || Header_1.TOKEN.test(rawValue)))) {
            handleInvalidValue(`Invalid parameter value: ${name}=${replacements[rawValue] || rawValue} does not match (token ( "=" ( token / quoted-string ))?). `, strict);
            continue;
        }
        let value = rawValue;
 ...
 */
function parseAcceptPart(part, replacements, strict) {
    ...
    if (!Header_1.SIMPLE_MEDIA_RANGE.test(range)) {
        handleInvalidValue(`Invalid Accept range: ${range} does not match ( "*/*" / ( token "/" "*" ) / ( token "/" token ) )`, strict);
        return;
    }
    let weight = 1;
    ...
    for (const { name, value } of parsedParams) {
        if (name === 'q') {
            ...
            map = extensionParams;
            if (!Header_1.QVALUE.test(value)) {
                handleInvalidValue(`Invalid q value for range ${range}: ${value} does not match ( "0" [ "." 0*3DIGIT ] ) / ( "1" [ "." 0*3("0") ] ).`, strict);
            }
 ...
 */
function parseNoParameters(input, strict = false) {
    ...
    return parts.map((part) => {
        ...
        if (qvalue) {
            ...
            if (!Header_1.QVALUE.test(val)) {
                handleInvalidValue(`Invalid q value for range ${range}: ${val} does not match ( "0" [ "." 0*3DIGIT ] ) / ( "1" [ "." 0*3("0") ] ).`, strict);
            }
 ...
 */
function parseAccept(input, strict = false) {
    ...
    return accepts.sort((left, right) => right.weight - left.weight);
}
 ...
 */
function parseAcceptCharset(input, strict = false) {
    const results = parseNoParameters(input);
    return results.filter((result) => {
        if (!Header_1.TOKEN.test(result.range)) {
            handleInvalidValue(`Invalid Accept-Charset range: ${result.range} does not match (content-coding / "identity" / "*")`, strict);
            return false;
        }
        return true;
    });
 ...
 * are ignored and not returned.
 */
function parseAcceptEncoding(input, strict = false) {
    const results = parseNoParameters(input);
    return results.filter((result) => {
        if (!Header_1.TOKEN.test(result.range)) {
 ...
 * are ignored and not returned.
 */
function parseAcceptLanguage(input, strict = false) {
    const results = parseNoParameters(input);
    return results.filter((result) => {
        // (1*8ALPHA *("-" 1*8alphanum)) / "*"
        if (result.range !== '*' && !/^[a-zA-Z]{1,8}(?:-[a-zA-Z0-9]{1,8})*$/u.test(result.range)) {
            handleInvalidValue(`Invalid Accept-Language range: ${result.range} does not match ((1*8ALPHA *("-" 1*8alphanum)) / "*")`, strict);
            return false;
        }
        return true;
    });
 ...
 */
function parseAcceptDateTime(input, strict = false) {
...
function parseContentType(input) {
    // Quoted strings could prevent split from having correct results
    const { result, replacements } = transformQuotedStrings(input);
    const [value, ...params] = result.split(';').map((str) => str.trim());
    if (!Header_1.SIMPLE_MEDIA_RANGE.test(value)) {
        logger.warn(`Invalid content-type: ${value}`);
        throw new BadRequestHttpError_1.BadRequestHttpError(`Invalid content-type: ${value} does not match ( token "/" token )`);
    }
    const contentType = new Header_1.ContentType(value);
...
const authSchemeRegexCache = new Map();
/**
 ...
 */
function matchesAuthorizationScheme(scheme, authorization) {
    const lowerCaseScheme = scheme.toLowerCase();
    if (!authSchemeRegexCache.has(lowerCaseScheme)) {
        authSchemeRegexCache.set(lowerCaseScheme, new RegExp(`^${(0, escape_string_regexp_1.default)(lowerCaseScheme)} `, 'iu'));
    }
...
exports.matchesAuthorizationScheme = matchesAuthorizationScheme;
/**
 ...
 */
function hasScheme(url, ...schemes) {
    const schemeOptions = new Set(schemes.map((item) => item.toLowerCase()));
