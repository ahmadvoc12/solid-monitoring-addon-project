...
const logger = (0, LogUtil_1.getLoggerFor)('StreamUtil');
/**
...
async function readableToString(stream) {
    return (await (0, arrayify_stream_1.default)(stream)).join('');
}
...
async function getSingleItem(stream) {
    const items = await (0, arrayify_stream_1.default)(stream);
    if (items.length !== 1) {
