...
class RepresentationMetadata {
    ...
    constructor(input, overrides) {
        ...
        if (overrides) {
            ...
            else if (overrides instanceof Header_1.ContentType) {
    ...
    setContentType(input) {
        ...
        if (typeof input === 'string') {
            ...
            if (!Header_1.SIMPLE_MEDIA_RANGE.test(input)) {
                // Not using an HttpError as HttpError depends on metadata
                throw new Error('Only simple content types can be set by string. Use the `contentTypeObject` function for complexer types.');
            }
            input = new Header_1.ContentType(input);
        }
    ...
    getContentType() {
        ...
        return new Header_1.ContentType(value, parameters);
    }
    removeContentType() {
