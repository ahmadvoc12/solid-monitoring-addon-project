...
function extractPersonalData(rdf) {
  ...
  /* ---- dpv:hasPersonalData ---- */
  rdf.match(/<https:\/\/w3id\.org\/dpv#hasPersonalData>[\s\S]*?;/g)
    ?.forEach(block => {
      block.match(/<https:\/\/w3id\.org\/dpv#[^>]+>/g)
        ?.forEach(iri => {
  ...
  /* ---- dpv:hasDataCategory (SENSITIVE FLAG) ---- */
  rdf.match(/<https:\/\/w3id\.org\/dpv#hasDataCategory>[\s\S]*?;/g)
    ?.forEach(block => {
      block.match(/<https:\/\/w3id\.org\/dpv#[^>]+>/g)
        ?.forEach(iri => {
          if (!result.dataCategories.includes(iri)) {
            result.dataCategories.push(iri);
          }
        });
    });

  /* ---- DATA VALUES (ONLY REAL DATA, NOT TIME) ---- */
  const valuePredicates = [
    /<https:\/\/schema\.org\/identifier>\s+"([^"]+)"/g,
    /<https:\/\/schema\.org\/bloodType>\s+"([^"]+)"/g
  ];

  valuePredicates.forEach(rx => {
    let m;
    while ((m = rx.exec(rdf)) !== null) {
      result.values.push(m[1]);
    }
  });

  result.sensitive = result.dataCategories.some(c =>
    c.includes("SensitivePersonalData") ||
    c.includes("SpecialCategoryPersonalData")
  );
...
/* ===============================
   AUDIT WRITER
================================ */
async function writeAudit({ pod, method, rdf, resource }) {
  if (!pod) return;
...
`;
  ...
  ttl += `
  ex:processingType "${parsed.sensitive ? "SensitiveDataAccess" : "NormalDataAccess"}" ;
